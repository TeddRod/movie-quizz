...
router.get("/generate_question", async (req, res) => {
  const showsAndMovies = Promise.all([
  ...
  let two_shows = [];
  for (let i = 0; i < 2; i++) {
    let one_random_movie =
      showsAndMovies[Math.floor(Math.random() * showsAndMovies.length)];

    two_shows.push(one_random_movie);

    showsAndMovies.splice(showsAndMovies.indexOf(one_random_movie), 1);
  }
  ...
  const movieOrTv = (show) => {
    return show.name ? "/tv/" : "/movie/";
  };

  await axios
    .get(
      `${base_url}${movieOrTv(quizz.show)}${
        quizz.show.id
    ...
    .catch((err) => {
      ...
      return res
        .status(500)
        .json({ message: "Erreur au chargment du casting !" });
    });
